...
from gymnax.environments import Pendulum, ContinuousMountainCar
...
rngs = nnx.Rngs(0, params=10, env=20, actions=30, optimize_samples=40)
...
algo = sac.SAC(VmapWrapper(env), hyperparameters)
...
# SAVE_PATH = path.abspath('examples/sac/_tmp/mountain_car_continuous')
...
from gymnax.visualize.vis_gym import render_pendulum, render_mountain_car_continuous
...
if VISUALIZE_METHOD == 'gif':
    ...
    vis.animate("./examples/sac/visualizations/sac_test_anim.gif")
    ...
    visualize_pygame(
        rngs, env, actor, 
        fps=FPS, 
        render_func=lambda state, action: render_pendulum(None, gymnax_env_params, state),
