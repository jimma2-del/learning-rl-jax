...
algo = td3.TD3(VmapWrapper(env))

import orbax.checkpoint as ocp

SAVE_PATH = path.abspath(f'examples/td3/_tmp/{ENV_NAME}')

# test load
#abstract_model = nnx.eval_shape(lambda: algo.make_actor(rngs=nnx.Rngs(0), noise=0))

def make_actor():
    rngs = nnx.Rngs(0)
    return algo.make_actor(rngs=rngs, noise=0)
...
if VISUALIZE_METHOD == 'video':
    ...
    mediapy.write_video(f"./examples/td3/visualizations/td3_{ENV_NAME}.mp4", frames, fps=FPS)
