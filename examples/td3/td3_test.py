...
gymnax_env = ContinuousMountainCar()
...
hyperparameters = td3.Hyperparameters(
    ...
    policy_delay = 2,
    exploration_noise = schedules.linear_schedule(1, 0.05, 0.1*STEPS), #0.1,
    target_noise = 0.2,
...
algo = td3.TD3(VmapWrapper(env), hyperparameters)
...
if VISUALIZE_METHOD == 'gif':
    ...
    vis.animate("./examples/td3/visualizations/td3_test_anim.gif")
    ...
    visualize_pygame(
        rngs, env, actor, 
        fps=FPS, 
        render_func=lambda state, action: render_mountain_car_continuous(None, gymnax_env_params, state),
