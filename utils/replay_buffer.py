...
class ReplayBuffer:
    def __init__(self, dummy_data_sample, capacity=10000):
        dummy_flattened, unravel_func = flatten_util.ravel_pytree(dummy_data_sample)
        ...
        self.sample_size = len(dummy_flattened)
        self.data_shape = (capacity, self.sample_size)
    ...
    @functools.partial(jax.jit, static_argnames=('self'))
    def insert(self, state: ReplayBufferState, samples):
        '''samples: farther back -> newer'''
    ...
    def sample(self, state: ReplayBufferState, num_samples, key):
...
if __name__ == "__main__":
    ...
    print(buffer.sample(buffer_state, 20, key))